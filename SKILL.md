...
Format: APA. In-text `[@key]`; References generated by `render_refs.py`. Math in LaTeX (`$...$` / `$$...$$`), never ASCII/Unicode spelling. Self-made figures → SVG into the topic's `figures/`. For a quantitative claim backed by several comparable effect sizes, `evidence_extract.py` pulls structured numbers (effect size + CI + N) out of the store's verified abstracts, and `evidence_figure.py` turns that into a grounded forest-plot SVG — both refuse to fabricate a data point that isn't actually in a verified entry.
...
**Not sure what's next?** `python scripts/conductor.py <topic> --json` is a stricter, machine-checkable alternative to `workflow_status.py`: it returns `gate: BLOCKED|done`, the specific blocking reasons, and ready-to-use params (round gaps, writing brief) for the next command — use it whenever you're unsure whether a phase boundary is actually clear to cross.
...
2. **Seed known DOIs** — `verify.py --add DOI TITLE YEAR AUTHORS --gap gap-N --source seed --round R`. Never hand-type DOIs from memory — copy from the source/landing page, or `search.py "<title>"` first. On `[ERROR] title_mismatch` (exit 2), verify.py prints candidate correct DOIs in stderr — pick the right one and re-add; don't reflexively `--force-mismatch` (a DOI pointing at another paper is the common case). Seeding a whole batch of landmark titles at once? `seed_sweep.py` resolves a list of titles to DOIs (Semantic Scholar fuzzy match) in one pass instead of one `search.py` call per title.
3. **Search** — `search.py "<query>"` (default `--source both` = CrossRef + Semantic Scholar; add `--source pubmed|biorxiv|clinicaltrials|all` to reach into `lib/extra_sources.py`'s adapters when a domain's literature lives outside CrossRef/S2, e.g. clinical trials registries or preprints). **Discovery-first**: run without `--auto-add`, eyeball titles, then `verify.py --add` the hits. Bounded auto-add (`--auto-add <topic> --gap gap-N --max-add N --rows 8`) only when you must; it's noisy (~75% reject rate unbounded).
4. **Genealogy (snowball)** — `genealogy.py <topic> --gap gap-N --round R --parallel 3`. OpenAlex ancestors + descendants, chained verify + abstract fetch. **Every declared round needs ≥1 genealogy entry.** Cap: early rounds wide (default 15, broad topics 20–25); final/confirmation round ≈ current eligible/10 (saturation triggers at new-verified/eligible ≤ 10%). Worried a whole cluster of relevant work never got cited into any seed? `gap_graph.py` clusters the citation graph as an advisory completeness check — surfaces candidate gaps the round-by-round search may have missed.
5. **Fetch missing abstracts** — `fetch.py <topic> --include abstract --parallel 3`. Scanned/image-only PDFs (old papers with no extractable text) fall back through `lib/ocr.py` to a MinerU CLI if one is installed (`HEALTH_REVIEW_OCR_CMD` or `mineru`/`magic-pdf` on PATH) — optional, degrades silently to `title_only` grounding when absent, never a hard dependency.
...
8. **Post-round triage** — read all analyst summaries; run `exclude.py <topic> DOI "reason"` on exclude_recommend, `regap.py <topic> DOI gap-M` on misfiled; any analyst returning Yes on (4) → open the next round. Noise that's merely abstract-only/title-only (not actually off-topic) belongs in the quarantine pool (`lib/quarantine.py`, `meta/quarantine.jsonl`) rather than a permanent `exclude` — quarantine can be revisited if a later round's better full-text access clears it, permanent exclude can't.
9. **Close the round** — `gaps_status.py <topic>` for per-gap state; write the research_log round section (gaps / targeted expansion / analyst four-list highlights / triage decisions — concise; `research_log.py scaffold <topic> --round R` auto-generates the mechanical parts — gap status table, per-source expansion counts, annotated four-list transcription — leaving only the `<!-- judgment -->` nodes for you to fill). Discipline: any gap with ≳5 new verified this round MUST get an analyst (step 7); skipping (only 1–2 weak entries, or a pure saturation-confirmation round) must be explicitly noted in research_log.
...
Steps 1-2 (and the write-side quality checks below) collapse into one command: `python scripts/finalize.py <topic>` runs lint → term_check → render_refs → `write_gate.py` → `md_to_html.py` in sequence and exits non-zero at whichever step blocks — use it as the default Phase 7 entrypoint instead of chaining the individual tools by hand.

**Optional deeper quality checks** (not required for `lint_review.py` to pass, but worth running before a topic you'll rely on):
- `faithfulness.py <topic>` — for every `[@key]`, checks the claim attached to it in the body is actually supported by that entry's abstract/full text (not just topically related). `faithfulness_audit.py` renders the results as a human-readable report. `write_gate.py` already enforces `suspect=insufficient=0` on the final draft; running `faithfulness.py` earlier (mid-round) catches a shaky claim before it's load-bearing in the prose.
- `recall_audit.py <topic>` — known-item recall audit: give it a list of papers you'd expect to be in the store (by title) and it reports which are missing, a check against the search/genealogy rounds having silently missed something obvious.
- `self_review.py <topic>` — generates a 7-question self-check template (scope creep, unaddressed gaps, citation density, etc.) for the main thread to fill before handoff; opt-in, not gated.
...
python scripts/conductor.py <topic> --json                        # stricter machine-checkable gate + next params
...
python scripts/finalize.py <topic>                                # lint+term_check+render+write_gate+html in one shot
# Optional quality / utility
python scripts/faithfulness.py <topic>                            # per-[@key] claim-vs-source support check
python scripts/recall_audit.py <topic>                            # known-item recall check against a title list
python scripts/gap_graph.py <topic>                               # citation-cluster completeness advisory
python scripts/dedupe.py <topic>                                  # find likely-duplicate store entries
python scripts/store_dump.py <topic>                              # flat table of verified entries for triage
```
...
`round_gate.py` / `write_gate.py` are separate, stricter mechanical gates layered on top of `lint_review.py` — `round_gate.py` checks a round's process (per-round analyst coverage, no permanent-exclude of merely-thin evidence) and `write_gate.py` checks the final draft (faithfulness `suspect=insufficient=0`, claim-map completeness, cross-gap reconciliation). Neither is wired into a Stop hook in this standalone skill (that requires host-level hook config this package doesn't ship) — run them explicitly via `finalize.py` or standalone when you want the stricter bar.

**Note on `HEALTH_REVIEW_DAEMON`:** the CLIs opt into an in-process daemon fast-path when `HEALTH_REVIEW_DAEMON=1` is set (`lib/daemon.py`), used by a separate host-level orchestration layer this skill doesn't ship. Leave it unset — every tool's default behavior (env var unset) runs directly and is byte-identical to the pre-daemon CLI, so nothing here depends on it.
